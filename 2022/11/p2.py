...
class Monkey:
    ...
    def take_turn(self, monkeys: Dict[Monkey], common_mod: int) -> None:
        for (
            old
        ) in (
            self.items
        ):  # The name old is special as it is used in the operation string
            new = eval(self.operation) % common_mod
...
def solve_problem_function(input_file: TextIO) -> str:
    ...
    common_mod = math.prod([x.test_div_by for x in monkeys.values()])
    for _ in range(10000):
        # print(f"_={_}")
        for key in monkeys:
            monkeys[key].take_turn(monkeys, common_mod)
